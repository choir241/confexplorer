...
export default function ConnectedUser({user}:{user: IUser}){
    ...
    return(
        ...
        <div className="userCard" key = {user.id}>
            <h4>{findUser.first_name} {findUser.last_name}</h4>
            <span>{user.had_coffee_chat ? <PiCoffeeFill/> : <PiCoffeeLight/>}</span>
            <span>{user.bookmark_connection ? <IoBookmark/> : <IoBookmarkOutline/>}</span>
        </div>
        : ""
        }
        </>
